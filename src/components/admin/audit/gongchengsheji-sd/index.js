...
class shejiShengding extends React.Component {
    constructor() {
        super();
    ...
    render() {
        ...
        if(this.props.res == null || this.props.res.审核的任务 == null){
            ...
            if(this.props.res.审定的任务.dataSize == undefined || this.props.res.审定的任务.dataSize == 0){
                return (
                    ...
                    <div className="MainRightContIn">
                        <div className="MainRightContIn-in">
                                    ...
                                    <tbody>
                                    {
                                        res.审定的任务.data.map((ele,i)=>{
                                            return <tr key={i} className={markId !== null && markId == ele.id ? 'active' : ''}>
                                                ...
                                                <td><Link to={'/audit/gongchengsheji-sd/'+ele.id} >{(i+1)+'、'+decodeURI(ele.acceptName)}</Link></td>
                            ...
                            <div className='Pagination'>
                                <Pagination showQuickJumper size='large' defaultCurrent={1} defaultPageSize={15} total={ResponseNum.审定的任务数量} onChange={this.pageChange} />
                            </div>
                        </div>
                    </div>
