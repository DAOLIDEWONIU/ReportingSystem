...
class gongchengjiaofei extends React.Component {
    ...
    render() {
        ...
        if(this.props.res == null){
            ...
            console.log('res.施工的任务',res.施工的任务)
            if(res.施工的任务 == undefined || res.施工的任务.dataSize == 0){
                return (
                        ...
                        <div className="MainRightContIn-in">
                            ...
                            <div className='MainRightCont-list'>
                                <table>
                                    <tbody>
                                    {
                                        this.props.res.施工的任务.data.map((ele,i)=>{
                                            return <tr key={i} className={markId !== null && markId == ele.id ? 'active' : ''}>
                                                ...
                                                <td><Link to={'/work/gongcheng-jiaofeiQR/'+ele.id} >{(i+1)+'、'+decodeURI(ele.acceptName)}</Link></td>
                            ...
                            <div className='Pagination'>
                                <Pagination showQuickJumper size='large' defaultCurrent={1} defaultPageSize={15} total={this.props.ResponseNum.施工的任务数量} onChange={this.pageChange} />
                            </div>
                        </div>
                    </div>
